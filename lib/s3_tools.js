...
exports.new = function(config){
	...
	function _get_existing(version){
		...
		return {date:null, notes:null, index:null}
	}

	function _write_version_info(cb){
		var data = {
			versions: versions,
			current: versions.length > 0 ? versions[versions.length-1].version : null
		};

		fs.writeFile(VERSIONS_FILE, JSON.stringify(data), function (err) {
			cb(err);
		});
	}
